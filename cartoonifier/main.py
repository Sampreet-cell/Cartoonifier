...
# cv2.imwrite('output.jpeg', cartoon)
...
layout = [[sg.Text('W E L C O M  E - T O - C A R T O O N I F I E R')],
          [sg.Text('Choose the line width : ')],
          [sg.Slider(range=(1, 50),
                     default_value=25,
                     size=(20, 15),
                     orientation='horizontal',
                     font=('Helvetica', 12))],
          [sg.Text('Choose the Blur Value : ')],
          [sg.Slider(range=(1, 15),
                     default_value=7,
                     size=(20, 15),
                     orientation='horizontal',
                     font=('Helvetica', 12))],
          [sg.Text('Choose the Total Colors : ')],
...
# Create the Window
window = sg.Window('Cartoonifier', layout)
...
while True:
    ...
    filename = sg.popup_get_file('Please Enter the image location  ')
    line_wdt = int(values[0])
    blur_value = int(values[1])
    totalColors = int(values[2])

    print(line_wdt, blur_value, totalColors)
